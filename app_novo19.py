...
class SCR:
        ...
        def getvalue(frame_entrada):
            ...
            if(isnumber(valor.get().replace(',', '.'))):
                ...
                if((valor_float<=max) and (valor_float >= 0)):
                    ...
                    # criando tabela
                    table = Table (frame, dataframe=frame_entrada_2[['Id', 'Nome', 'SCR Total', 'Telefone']], align = 'center', width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)
                    ...
                    table.columncolors[lista_colunas[6]] = '#dcfcfb'  
        ...
        def getvalue(frame_entrada):
            ...
            if(isnumber(valor.get().replace(',', '.'))):   
                ...
                if((valor_float>=min) and (valor_float >= 0)):
                    ...
                    table.columncolors['SCR Total'] = '#dcfcfb' 
    ...
    def scr_consulta(self, df):  
        ...
        def sel_1(frame_entrada, coluna, title):
            ...
            # enfase na coluna scr
            table.columncolors[lista_colunas[2]] = '#dcfcfb' 
...
class limite:
    def limite_uso(self, df, filtro): 
        ...
        # enfase na coluna
        table.columncolors['Usando Limite'] = '#dcfcfb'
...
class graficos:
    ...
    def grafico_pie_count(self, df, coluna, valor1, valor2, title): 
        entrada_sim = df[coluna] == valor1 
        entrada_nao = df[coluna] == valor2

        frame_entrada_sim = df[entrada_sim]
        frame_entrada_nao = df[entrada_nao]
        # agrupador: count, sum, etc
        x = frame_entrada_sim[coluna].count()
        y = frame_entrada_nao[coluna].count()
        
        slice = [x, y]
        
        labels = [valor1, valor2]
        explode = [0.2, 0] #vou destacar o terceiro

        grafico = plt.pie(slice, explode=explode, labels=labels, shadow=True, autopct = '%.2f%%')
        plt.title(title)
        plt.show()

    def grafico_pie_sum(self, df, coluna1, coluna2, title):
        # Somando os valores da coluna2
        x = df[coluna1].sum()
        ...
        # Ênfase no setor do gráfico
        explode = [0.2, 0]

        # Criando o gráfico
        plt.pie(slice, explode=explode, labels=labels, shadow=True, autopct = '%.2f%%')
    ...
    def histograma(self, df, coluna, title, xlabel):  
        # bins = 5?
        plt.hist(df[coluna], bins=9, histtype='barstacked')
        plt.title(title, fontweight='bold')
        plt.xlabel(xlabel, fontweight='bold')
    ...
    def cut_classes(self, faixa, title, xlabel):             
        plt.hist(faixa, label=xlabel, bins=9)
        plt.title(title, fontweight='bold')
        plt.xlabel(xlabel, fontweight='bold')
        plt.ylabel('Frequência Absoluta', fontweight='bold')   
    ...
    def filtro_total_ano(self, df):    
        def sel(year, df):           
            ...
            #graficos.boxplot_subplot2(self, grafico, grafico, 'grafico1', 'grafico2') 
        ...
        frame1 = LabelFrame(root, text="Selecionar Ano: ", font=80, bd=3)
...
class investimentos:   
    ...
    def investimento_maior_que(self):       

        def getvalue():
           
            if(isnumber(valor.get())):
                max = df['investimento'].max()        
                    #for e in df['investimento'].values:           
                        #if (e > max):                
                            #max = e
                            #if(len(L)>0):
                        #df_mod = pd.DataFrame(L)
                if((valor.get()<max)and (valor.get()>=0)):
                    frame_entrada = df[df['investimento'] > valor.get()]
                        #entrada = df[entrada]
                    tela_1 = Tk()      
                    tela_1.title("Dados")
                    tela_1.state("zoomed")
                    ...
                    table = Table (frame, dataframe=frame_entrada[['Id', 'Nome', 'Investimento', 'Telefone']], width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)
                                
                    table.show()
                    tela_1.mainloop()
                elif(valor.get()<0):
                    tela3 = Tk()
                    tela3.title("Valores negativos")
                    tela3.state('zoomed')
                    label = Label(tela3, text = "Digite um valor maior ou igual a zero. ", font = 'Arial 20').grid()
                    tela3.mainloop()
                else:
                    tela_2 = Tk()
                    tela_2.title('Mensagem')
                    tela_2.state('zoomed') 
                    texto = StringVar(tela_2)
                    texto.set(f"Não constam clientes com valores maiores que {max:0.2f}")           
                    label_nao_encontrado = Label(tela_2, textvariable= texto, font = 'Arial 20').grid()
                    tela_2.mainloop()
           
        
        tela = Tk()
        valor = DoubleVar(tela)    
        
        tela.title("Maior que: ")
        tela.state("zoomed")   

        frame = LabelFrame(tela, text = 'Investimento: ', padx = 20, pady = 20) 
        frame.place(x = 500, y = 120)        
    
        label_investimento = Label(frame, text = "Maior que: ")             
        

        e1 = Entry(frame,textvariable = valor,width=30,fg="blue",bd=3,selectbackground='violet')
        
        button_profissao = Button(frame, 
            text='Pesquisar', 
            fg='White', 
            bg= 'Dark blue',height = 1, command=getvalue)
       

        label_investimento.grid(row = 0, column = 0)
        e1.grid(row = 0, column = 1)
        button_profissao.grid(row = 1, columnspan = 2, sticky = 'we')

       
    def investimento_menor_que(self):
        def getvalue():
            min = df['investimento'].min()
            #for e in df['investimento'].values:           
                #if (e < min):                
                    #min = e
                    #if(len(L)>0):
                #df_mod = pd.DataFrame(L)
            if((valor.get()>min) and (valor.get() >= 0)):
                frame_entrada = df[df['investimento'] < valor.get()]
                #entrada = df[entrada]
                tela_1 = Tk()      
                tela_1.title("Dados")
                ...
                frame_entrada = frame_entrada.rename(columns={'investimento': 'Investimento'}) 
                frame_entrada = frame_entrada.rename(columns={'telefone': 'Telefone'})        
                table = Table (frame, dataframe=frame_entrada[['Id', 'Nome', 'Investimento', 'Telefone']], floatprecision=2, align='center',width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)                        
                table.columncolors['Investimento'] = '#dcfcfb'
                table.show()
                tela_1.mainloop()
        ...
        tela.title("Menor que: ")
        tela.state("zoomed")   

        frame = LabelFrame(tela, text = 'Investimento: ', padx = 20, pady = 20) 
        frame.place(x = 500, y = 120)        
        
        label_investimento = Label(frame, text = "Menor que: ")
    ...
    def investimento_maximo(self):
        max = df['investimento'].max()  
        #max_str = str(max)
        tela = Tk()
        maximo = DoubleVar(tela)
        maximo.set(f"{max:.2f}")    
        
        tela.title("Maior valor de investimento")
        tela.state("zoomed")   

        frame = LabelFrame(tela, text = 'Máximo: ', padx = 20, pady = 20) 
        frame.place(x = largura//2, y = (altura//2)-200)        
        
        label_maximo = Label(frame, textvariable = maximo).grid()


        frame1 = LabelFrame(tela, text = 'Cliente(s) com valor máximo: ', padx = 20, pady = 20) 
        frame1.place(x = largura//2, y = (altura//2)-80)   

        def getvalue():
            tela_1 = Tk()
            tela_1.state('zoomed')
            tela_1.title("Cliente(s) com os maiores investimentos")
            frame_1 = Frame(tela_1)
            frame_1.grid()
            valor_max = df['investimento'] == max
            frame_entrada = df[valor_max]
            frame_entrada = frame_entrada.rename(columns={'id': 'Id'}) 
            frame_entrada = frame_entrada.rename(columns={'nome': 'Nome'}) 
            frame_entrada = frame_entrada.rename(columns={'investimento': 'Investimento'}) 
            frame_entrada = frame_entrada.rename(columns={'telefone': 'Telefone'}) 
            table = Table (frame_1, dataframe=frame_entrada[['Id', 'Nome', 'Investimento', 'Telefone']], width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)
            table.show()


        button_maximo = Button(frame1, 
                text='Pesquisar', 
                fg='White', 
                bg= 'Dark blue',height = 1, width = 18, command = getvalue).grid()
    ...
    def investimento_minimo(self):
        min = df['investimento'].min()  
        #max_str = str(max)
        tela = Tk()
        minimo = DoubleVar(tela)
        minimo.set(f"{min:.2f}")    
        
        tela.title("Menor valor de investimento")
        tela.state("zoomed")   

        frame = LabelFrame(tela, text = 'Mínimo: ', padx = 20, pady = 20) 
        frame.place(x = largura//2, y = altura//2 - 200)        
        
        label_minimo = Label(frame, textvariable = minimo).grid()

        frame1 = LabelFrame(tela, text = 'Cliente(s) com valor mínimo: ', padx = 20, pady = 20) 
        frame1.place(x = largura//2, y = (altura//2)-80)   

        def getvalue():
            tela_1 = Tk()
            tela_1.state('zoomed')
            tela_1.title("Cliente(s) com os menores valores em investimentos")
            frame_1 = Frame(tela_1)
            frame_1.grid()
            valor_min = df['investimento'] == min
            frame_entrada = df[valor_min]
            frame_entrada = frame_entrada.rename(columns={'id': 'Id'}) 
            frame_entrada = frame_entrada.rename(columns={'nome': 'Nome'}) 
            frame_entrada = frame_entrada.rename(columns={'investimento': 'Investimento'}) 
            frame_entrada = frame_entrada.rename(columns={'telefone': 'Telefone'})             
        ...
        #for i, e in enumerate(df['investimento']):
            #if e == min:
                #print(df.iat[i, 1], e)

    # Exibir investimentos em ordem crescente
    def invest_cresc_desc(self):  
        def sel_1():
            #crescente = StringVar()
            #crescente.set(df.sort_values(by='investimento'))
            #print(crescente)
            ...
            frame_entrada = frame_entrada.rename(columns={'investimento': 'Investimento'}) 
            frame_entrada = frame_entrada.rename(columns={'telefone': 'Telefone'})       
            table = Table (frame, dataframe=frame_entrada[['Id', 'Nome', 'Investimento', 'Telefone']].sort_values(by='Investimento'), width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)
            table.show()
            tela_1.mainloop()   
        ...
        def sel_2():
            ...
            frame_entrada = frame_entrada.rename(columns={'investimento': 'Investimento'}) 
            frame_entrada = frame_entrada.rename(columns={'telefone': 'Telefone'})        
            table = Table (frame, dataframe=frame_entrada[['Id', 'Nome', 'Investimento', 'Telefone']].sort_values(by='Investimento', ascending = False), width = (largura - 100), height = (altura - 50), showtoolbar=False, showstatusbar=False)
            table.show()
            tela_1.mainloop()
        ...
        frame1 = LabelFrame(root, text="Crescente x Decrescente")
        frame1.place(x=largura//2, y = (altura//2)-200)

        var = IntVar(frame1)
        R1 = Radiobutton(frame1, text="Crescente", variable=var, value=1,command=sel_1)
        R1.grid(sticky = W)

        R2 = Radiobutton(frame1, text="Decrescente", variable=var, value=2,command=sel_2)
        R2.grid(sticky = W)
    ...
    def investimento_grafico_relplot(self, df):
        # criando frame local
        frame_entrada = df.copy()
        #area, axes = plt.subplots(3, 4, sharex=True, figsize=(16,8)) 
    ...
    def clientes_profissao(self, df):  
        def getvalue():
            ...
            if(str(profissao.get().rstrip().lstrip().lower()) in df['profissao'].values):
                ...
                # destacando coluna
                table.columncolors['Profissão'] = '#dcfcfb'
...
class restricoes:
    ...
    def restricoes_possui(self,df, filtro):
        ...
        # destacando coluna
        table.columncolors['Restrição'] = '#dcfcfb'
    ...
    def sem_restricao_pagou_parcial_cartao(self, df, coluna1, filtro_col1, coluna2, filtro_col2):
        ...
        # destacando coluna
        table.columncolors['Pgto.Parcial Fatura'] = '#dcfcfb'
    ...
    def sem_restricao_aposentado(self, df, coluna1, filtro_col1, coluna2, filtro_col2):        
        ...
        # destacando coluna
        table.columncolors['Aposentado'] = '#dcfcfb'
    ...
    def grafico_restricoes_sim_nao(self, df): 
        ...
        # grafico ajustavel a tela
        plt.tight_layout()

        # adicionar titulo na figura
        #plt.suptitle('Cliente com Histórico de Restrição',fontsize=10)   
...
class estatisticas:
    def estatisticas(self): 
        ...
        def botao_Click():
            ...
            table.columncolors['Estatísticas'] = '#dcfcfb' 
...
clientes_menu.add_command(label = "Abrir arquivo", command = lambda: clientes.abrir_arquivo(menu_inicial))
...
investimentos_menu.add_command(label = "Crescente x Decrescente", command = lambda: investimentos.invest_cresc_desc(menu_inicial))
investimentos_menu.add_command(label = "Grafico: Relação", command = lambda: investimentos.investimento_grafico_relplot(menu_inicial, df))
